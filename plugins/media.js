...
cmd({
    pattern: "wallpaper",
    desc: "Search for wallpapers based on your choice.",
    react: "🖼️",
    category: "utility",
    use: ".wallpaper <keyword>",
    filename: __filename,
}, async (conn, mek, m, { args, reply, sendMedia }) => {
    try {
        if (args.length === 0) {
            return reply("❌ Please provide a keyword to search for wallpapers.\nExample: `.wallpaper nature`");
        }

        const keyword = args.join(" "); // Combine all arguments into a search keyword
        reply(`🔍 Searching for wallpapers related to *${keyword}*...`);

        // Fetch wallpapers from the API
        const response = await axios.get(`https://davidcyril-api.onrender.com/wallpapers?search=${encodeURIComponent(keyword)}`);
        const wallpapers = response.data; // Response is expected to be an array of URLs

        if (!wallpapers || wallpapers.length === 0) {
            return reply(`❌ No wallpapers found for *${keyword}*. Please try a different keyword.`);
        }

        // Send the first wallpaper with the caption
        await sendMedia(
            mek.chatId, // Chat ID to send the media
            wallpapers[0], // First wallpaper URL
            {
                caption: `🖼️ *Wallpaper results by Kerm Md V4*\n\n*Keyword*: ${keyword}`,
            }
        );
    } catch (error) {
        console.error("Error fetching wallpapers:", error.message);
        reply("❌ An error occurred while searching for wallpapers. Please try again later.");
    }
});