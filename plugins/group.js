...
const welcomeMessages = {}; // Stocke les messages d'accueil temporairement

cmd({
    pattern: "setwelcome",
    desc: "Set a welcome message for new members in the group.",
    category: "group",
    react: "✨",
    filename: __filename
    ...
    if (!args[0]) {
        return reply(
            ...
            "*Optional:* Add an image link: .setwelcome Welcome @user! 🎉\nhttps://example.com/image.jpg\n\n" +
            "📌 Use `@user` to mention the new member."
        );
    }

    const input = args.join(" ").split("\n");
    const message = input[0]; // Premier argument = le message
    const imageUrl = input[1] || null; // Deuxième argument = lien d'image (facultatif)

    // Sauvegarde le message d'accueil pour ce groupe
    welcomeMessages[groupId] = { message, image: imageUrl };

    reply(
        `✅ *Welcome message has been set successfully!*\n\n` +
        `📌 *Message:* ${message}\n` +
        (imageUrl ? `🖼️ *Image:* ${imageUrl}` : "No image link provided.")
    );
});
...
conn.on("group-participants-update", async (conn, mek, m) => {
    ...
    const welcome = welcomeMessages[id];
    const message = welcome.message.replace(/@user/g, `@${participants[0].split("@")[0]}`);
    const imageUrl = welcome.image;
...
const goodbyeMessages = {}; // Stocke les messages d'adieu temporairement

cmd({
    pattern: "setgoodbye",
    desc: "Set a goodbye message for members who leave the group.",
    category: "group",
    react: "👋",
    filename: __filename
}, async (conn, mek, m, { isGroup, isAdmin, isOwner, args, reply }) => {
    ...
    if (!args[0]) {
        return reply(
            ...
            "*Optional:* Add an image link: .setgoodbye Goodbye @user! 😢\nhttps://example.com/image.jpg\n\n" +
    ...
    // Sauvegarde le message d'adieu pour ce groupe
    goodbyeMessages[groupId] = { message, image: imageUrl };

    reply(
        `✅ *Goodbye message has been set successfully!*\n\n` +
        `📌 *Message:* ${message}\n` +
        (imageUrl ? `🖼️ *Image:* ${imageUrl}` : "No image link provided.")
    );
});
...
conn.on("group-participants-update", async (conn, mek, m) => {
    ...
    const goodbye = goodbyeMessages[id];
    const message = goodbye.message.replace(/@user/g, `@${participants[0].split("@")[0]}`);
    const imageUrl = goodbye.image;

    // Envoie le message d'adieu
    await conn.sendMessage(id, {
        text: message,
        mentions: [participants[0]],
        ...(imageUrl && { image: { url: imageUrl } })
